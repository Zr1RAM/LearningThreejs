...
export default class LanesController {
    ...
    initializeLaneSplines() {
       ...
       for (let i = 0 ; i < 5 ; i++) {
            this.laneFrontSplines.push(
                makeSplineFromPoints(points, 0xffffff)
            );
    ...
    makeSplineFromPoints(points,splineColor) {
        ...
        return new Line2( this.updateLaneSplineGeometry(points), laneMaterial );
    }
    
    
    updateLaneSplineGeometry(points) {
    ...
    update() {
        //console.log("lanesController's tick");
    ...
    updateLaneSplinePoints(data) {
        ...
        if (intervals > 0) {
            ...
            for (let i = 0; i < intervals - 2; i++) {
                ...
                laneCoordinates.push(
                    makeCoordinateForLaneSpline(yCoordinate, 0, data._range_start_m + (this.intervalValue * i))
                );
            }
